...
** theme
#+begin_src emacs-lisp
(defun 2c-follow-system-theme ()
  (let ((system-theme
	 (cond ((eq system-type 'darwin)
		(if (string= (shell-command-to-string "defaults read -g AppleInterfaceStyle") "Dark\n")
		    'dark 'light))
	       (t 'dark))))
    (cond ((and (eq system-theme 'dark)
		(not (eq frame-background-mode 'dark)))
	   (disable-theme 'sanityinc-tomorrow-day)
	   (load-theme 'sanityinc-tomorrow-bright t))
	  ((and (eq system-theme 'light)
		(not (eq frame-background-mode 'light)))
	   (disable-theme 'sanityinc-tomorrow-bright)
	   (load-theme 'sanityinc-tomorrow-day t)))))

(use-package color-theme-sanityinc-tomorrow
  :init
  ;; if we load-theme directly, the frame only gets half the theme
  ;; loaded.  Delayed loading fixes this issue.
  (run-at-time "0.2" 60 #'2c-follow-system-theme)
...
(use-package doom-modeline
  ...
  (doom-modeline-def-modeline 'main
    ...
    '(compilation objed-state misc-info project-name persp-name battery grip github debug repl lsp minor-modes input-method indent-info buffer-encoding process vcs check time))

  (setq doom-modeline-vcs-display-function
	(lambda ()
	  (let* ((name (doom-modeline-vcs-name))
		 (displayed-name (if (or (string= name "master")
					 (string= name "main"))
				     ""
				   name)))
	    displayed-name)))
...
(use-package aidermacs
  ...
  (aidermacs-editor-model "openrouter/deepseek/deepseek-r1:free")
  (aidermacs-weak-model "openrouter/deepseek/deepseek-chat:free")
  )
...
(use-package adoc-mode
  ...
  (add-to-list
   ...
   `("\\.adoc\\>" .
	...
	(start-process (concat "brave " url) nil "open" "-a" "Brave Browser" url)))))
#+end_src
...
(use-package erc
  ...
  (setq erc-log-channels-directory nil)
  (setq erc-modules '(completion
		      ...
		      list
		      scrolltobottom))
  :hook ((erc-mode . (lambda () (subword-mode 0)))
	 ;(erc-mode . erc-add-scroll-to-bottom)
	 (erc-mode . 2c-erc-set-scroll-conservatively)))
...
(use-package projectile
  ...
  :config
  (defun 2c-projectile-ignore-homedir (dir)
    (if (equal dir (expand-file-name "~/"))
	nil
      dir))
  (advice-add 'projectile-locate-dominating-file :filter-return #'2c-projectile-ignore-homedir))

(defun 2c-project-ignore-homedir (project)
  (if (and project
	   (equal (expand-file-name (nth 2 project)) (expand-file-name "~/")))
      nil
    project))
(advice-add 'project-try-vc :filter-return #'2c-project-ignore-homedir)
#+end_src
...
;; set working directory for python-ruff.  However, for unknown
;; reasons we can't do this in :config and have to do it delayed like
;; so.
(require 'flycheck)
(setf (flycheck-checker-get 'python-ruff 'working-directory)
      'flycheck-python-find-project-root)
#+end_src
...
(use-package ruby-test-mode
  :preface
  (defun ruby-test-run-all ()
    (interactive)
    (ruby-test-with-ruby-directory
     (or (ruby-test-find-file) default-directory)
     (ruby-test-run-command (ruby-test-spec-command ""))))
  :bind (:map ruby-test-mode-map
	      ("C-c C-t C-t" . ruby-test-run-all)))
#+end_src
...
(use-package rustic
  ...
  :bind (:map rustic-mode-map
              ...
              ("C-c C-c s" . consult-lsp-file-symbols)
